...
class PGExplainer(BaseExplainer):
    ...
    """
    def __init__(self, model_to_explain, graphs, features, task, epochs=30, lr=0.000003, temp=(5.0, 2.0), reg_coefs=(0.05, 1.0),sample_bias=0):
    ...
    def _create_explainer_input(self, pair, embeds, node_id):
        ...
        """
        rows = pair[0].type(torch.LongTensor)
        cols = pair[1].type(torch.LongTensor)
    ...
    def train(self, indices = None):
        ...
        for e in tqdm(range(0, self.epochs)):
            ...
            loss.backward()
            #print('here')
            #torch.nn.utils.clip_grad_value_(self.explainer_model.parameters(), 1)
            optimizer.step()
