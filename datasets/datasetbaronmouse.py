...
"""
DATA_FOLDER="/gpfs/data/rsingh47/hzaki1/data-baron-mouse/"
RESOURCES_FOLDER="/gpfs/data/rsingh47/hzaki1/data-baron-mouse/resources"
DATABASE_FOLDER = "/gpfs/data/rsingh47/hzaki1/data-baron-mouse/databases"
LABELS = os.path.join(RESOURCES_FOLDER, 'Labels.csv')
...
SC_EXP_FNAME = os.path.join(RESOURCES_FOLDER, "Filtered_MousePancreas_data.csv")
REGULONS_FNAME = os.path.join(DATA_FOLDER, "regulons.p")
MOTIFS_FNAME = os.path.join(DATA_FOLDER, "motifs.csv")



class BaronMouseDataset(Dataset):
    def __init__(self, root, transform=None, pre_transform=None):
        """
        root = Where the dataset should be stored. This folder is split
        into raw_dir (downloaded dataset) and processed_dir (processed data). 
        """
        self.exmatrix = pd.read_csv(SC_EXP_FNAME, header=0).drop(columns=['Unnamed: 0'])
        ...
        self.cellToIndex = {}
        for i, cell in enumerate(self.labels['x'].unique()):
            self.cellToIndex[cell] = i
        self.indexToCell = {y:x for x,y in self.cellToIndex.items()}

        super(BaronMouseDataset, self).__init__(root, transform, pre_transform)
    ...
    def processed_file_names(self):
        ...
        return [f'data_{i}.pt' for i in range(len(list(self.labels["x"])))]
    ...
    def process(self):
        ...
        for index, row in tqdm(self.exmatrix.iterrows(), total=self.exmatrix.shape[0]):
            ...
            edge_index = self.adj
            # Get labels info
            label = self.cellToIndex[self.labels.values[index][0]]
